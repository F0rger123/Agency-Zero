  ...
  if (!response.data) {
    return (
      <main className="mx-auto max-w-2xl px-6 py-16">
        ...
        <h1 className="mt-4 text-2xl font-semibold">Contract not found</h1>
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-12 sm:py-20">
      <header className="border-b border-border pb-10">
        ...
        <p className="mt-2 text-sm text-muted-foreground">For {contract.client_name}{contract.company ? `, ${contract.company}` : ""} · Version {contract.version}</p>
      </header>
      <article className="mt-10 whitespace-pre-wrap text-sm leading-7">{contract.body}</article>
      {contract.status === "sent" ? (
        <SignContractForm token={token} />
      ) : (
        <p className="mt-10 border-t border-border pt-6 text-sm font-medium">
          This contract is {contract.status}
          {contract.signer_name ? ` and was signed by ${contract.signer_name}` : ""}
      ...
      <footer className="mt-16 border-t border-border pt-5 text-xs text-muted-foreground">
        Electronic signing is recorded with the signer name, a timestamp, and a preserved snapshot of the signed document.
      </footer>
    </main>
  );
}
